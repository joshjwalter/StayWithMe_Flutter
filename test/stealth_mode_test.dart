...
void main() {
  group('Stealth Mode UI', () {
    ...
    testWidgets('Stealth mode toggle is only visible in idle state', (tester) async {
      ...
      final mockApiClient = AlarmApiClient(baseUrl: '');

      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: AlarmPage(apiClient: mockApiClient),
          ),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('Stealth mode resets to OFF after timer completes', (tester) async {
      final mockApiClient = AlarmApiClient(baseUrl: '');
